      ...
      "outputs": [
        {
          ...
          "internalType": "struct Splitwise.Ownership[]",
          "name": "",
          "type": "tuple[]"
        }
      ],
...
async function getLeastAmount(path) {
	let amount = 0;
...
async function getUsers() {
	const result = await BlockchainSplitwise.getAllOwingData();
	return result;
}
...
async function getTotalOwed(user) {
	...
	let total = 0;
	for (let i = 0; i < result.length; i++) {
		console.log("Owner Address: ", result[i].owingAddress, "Amount: ", result[i].amount);
...
async function add_IOU(creditor, amount) {
	...
	const cycle = await doBFS(defaultAccount, creditor, owingDataAdapter);
	if (!cycle) {
		console.log("No cycle found");
		return;
	}
	console.log("Cycle found: ", cycle);
	const leastAmount = await getLeastAmount(cycle);
	console.log("Least Amount: ", leastAmount);

	for (let i = 1; i < cycle.length; i++) {
		console.log("Debtor: ", cycle[i - 1],"Creditor: ", cycle[i], "Amount: ", leastAmount);
		await BlockchainSplitwise.iou(cycle[i - 1], cycle[i], leastAmount);
	}
}
